...
$app->put('/projects', function (Request $req, Response $res) use ($conn) {
    ...
    $stmt->bind_param("issssssssssiiissssss", $post["client_id"], $post["name"], $post["status"], $post["hosting"], $post["github_url"], $post["drive_url"], $post['project_url'], $post['project_login_url'], date("Y-m-d H:i:s"), date("Y-m-d H:i:s"), $post["pandle_id"], $post["completion_amount"], $post["bb_revenue"], $post["bb_expenses"], $post["viewer"], $post["contributor"], $post["admin"], $post["enquiry_date"], $post["start_date"], $post["completion_date"]);
    $stmt->execute();
    $stmt->close();
...
$app->post('/projects', function (Request $req, Response $res) use ($conn) {
    ...
    $stmt = $conn->prepare("UPDATE projects SET name = ?, status = ?, hosting = ?, github_url = ?, drive_url = ?, project_url = ?, project_login_url = ?, updated_at = ?, pandle_id =?, completion_amount = ?, bb_revenue = ?, bb_expenses = ?, viewer = ?, contributor = ?, admin = ?, enquiry_date = ?, start_date = ?, completion_date = ? WHERE id = ?");
    $stmt->bind_param("sssssssssiiissssssi", $post["name"], $post["status"], $post["hosting"], $post["github_url"], $post["drive_url"], $post['project_url'], $post['project_login_url'], date("Y-m-d H:i:s"), $post["pandle_id"], $post["completion_amount"], $post["bb_revenue"], $post["bb_expenses"], $post["viewer"], $post["contributor"], $post["admin"], $post["enquiry_date"], $post["start_date"], $post["completion_date"], $post["id"]);
    $stmt->execute();
    $stmt->close();
