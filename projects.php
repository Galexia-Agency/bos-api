...
$app->put('/projects', function (Request $req, Response $res) use ($conn) {
    ...
    $stmt->bind_param("isssssssssssssssssssis", $post["client_id"], $post["name"], $post["status"], $post["hosting"], $post["php"], $post["github_url"], $post["drive_url"], $post['project_url'], $post['project_login_url'], date("Y-m-d H:i:s"), date("Y-m-d H:i:s"), $post["pandle_id"], $post["completion_amount"], $post["bb_revenue"], $post["bb_expenses"], $post["viewer"], $post["contributor"], $post["admin"], $post["enquiry_date"], $post["start_date"], $post["ongoing"], $post["completion_date"]);
    $stmt->execute();
    $stmt->close();
...
$app->post('/projects', function (Request $req, Response $res) use ($conn) {
    ...
    $stmt->bind_param("ssssssssssssssssssisi", $post["name"], $post["status"], $post["hosting"], $post["php"], $post["github_url"], $post["drive_url"], $post['project_url'], $post['project_login_url'], date("Y-m-d H:i:s"), $post["pandle_id"], $post["completion_amount"], $post["bb_revenue"], $post["bb_expenses"], $post["viewer"], $post["contributor"], $post["admin"], $post["enquiry_date"], $post["start_date"], $post["ongoing"], $post["completion_date"], $post["id"]);
    $stmt->execute();
    $stmt->close();
